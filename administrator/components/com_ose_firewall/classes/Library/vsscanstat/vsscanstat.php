...
class oseVsscanStat {
    ...
    public function getMalwareMapDB($search, $status, $start, $limit, $sortby, $orderDir)
    {
        ...
        // Get Records Query;
        $temp = $this->getAllRecords ($where);
        $return['data'] = $this->getFormattedScanResults($temp);
    ...
    public function getFormattedScanResults($records)
    {
        $final_Result = array();
        if(empty($records))
        {
            ...
            foreach($records as $key=>$val)
            {
                ...
                $final_Result[$key]->size = $this->getyFileSize($val->filename);
            }
            return $final_Result;
        }
    }
    public function getyFileSize($file)
    {
        if(file_exists($file))
        {
            return $this->filesize_formatted($file);
        }else{
            return "0.00 KB";
        }
    }
    ...
    public function exportMarkAsCleanFile()
    {
        ...
        $this->updateMarkAsCleanList($result);
    }

    private function updateMarkAsCleanList($result)
    {
        $filePath = CENTRORABACKUP_FOLDER . ODS . "markasclean.php";
        if(file_exists($filePath))
        {
            $whitelist = $this->getWhiteListFiles();
            $temp = array_merge($whitelist,$result);
            file_put_contents($filePath, '<?php $whitelist = ' . var_export($temp, true) . ';');
        }else {
            file_put_contents($filePath, '<?php $whitelist = ' . var_export($result, true) . ';');
        }
    }

    private function getWhiteListFiles()
    {
        $whitelist = null;
        $filePath = CENTRORABACKUP_FOLDER . ODS . "markasclean.php";
        if(file_exists($filePath)){
