...
class oseFirewallUploadManager
{
    ...
    public function getFileUploadLogV7()
    {
        $columns = oRequest::getVar('columns', null);
        $limit = oRequest::getInt('length', 15);
        $start = oRequest::getInt('start', 0);
...
//        $status = $columns[4]['search']['value'];
        ...
        $result = $this->getFileUploadDbFromDB($search['value'],$start, $limit, $sortby, $orderDir);
        return $result;
    }
    public function getFileUploadDbFromDB($search, $start, $limit, $sortby, $orderDir)
    {
        $return = array();
        if (!empty($search)) {
            $this->getWhereNameLogV7($search);
        }
//        if (!empty($status)) {
//            $this->getWhereStatus($status);
//        }
...
//        }
        ...
        $where = $this->db->implodeWhere($this->where);
        $return= $this->getAllRecordsLogV7($where);
        return $return;
    }

    public function getAllRecordsLogV7($where)
    {
        $sql = 'SELECT * FROM `#__osefirewall_fileuploadlogv7`';
        $query = $sql . $where . $this->orderBy . " " . $this->limitStm;
        $this->db->setQuery($query);
//        print_r( $this->db->setQuery($query));exit;
        $temp = $this->db->loadResultList();
        $result = $this->formatFileUploadlogV7($temp);
        $count = $this->getAllCountsLogV7($where);
        $result['recordsTotal'] = $count['recordsTotal'];
        $result['recordsFiltered'] = $count['recordsFiltered'];
        return $result;
    }

    private function getAllCountsLogV7($where)
    {
        $return = array();
        // Get total count
//        $attrList = array("COUNT(`ext_id`) AS count");
        $sql = 'SELECT COUNT(`id`) AS count FROM `#__osefirewall_fileuploadlogv7`';
        $this->db->setQuery($sql);
        $result = $this->db->loadObject();
        $return['recordsTotal'] = $result->count;
        // Get filter count
    ...
    public function formatFileUploadlogV7($log)
    {
        $result = array();
        $i= 0;
        foreach($log as $record)
        {
            ...
            $result[$i]['ip'] = $record['ip'];
            $result[$i]['filename'] = $record['filename'];
            $result[$i]['filetype'] = $record['filetype'];
            $result[$i]['validationstatus'] =  $this->getLogStatusIcon($record['validationstatus']);
            $result[$i]['virusscanstatus'] = $record['virusscanstatus'];
            $result[$i]['datetime'] = $record['datetime'];
            $i++;
        }
        $temp['data']= $result;
//        $temp['recordsFiltered'] = count($result);
//        $temp['recordsTotal'] =count($result);
        return $temp;
    }



    private function getWhereNameLogV7($search)
    {
        $this->where[] = "`ip` LIKE " . $this->db->quoteValue($search . '%', true) . " OR `filename` LIKE " . $this->db->quoteValue($search . '%', true). " OR `filetype` LIKE " . $this->db->quoteValue($search . '%', true);
    }
    ...
    private function getLogStatusIcon($status)
    {
        switch ( $status ){
            ...
            case 2 :
                ...
                break;
            case 3 :
                $status = '<div class="color-red"><i class="glyphicon glyphicon-alert"></i> ' . oLang:: _get('BLOCKED_SUS_UPLOAD_LOG') . '</div>';
                break;
            case 4 :
                $status = '<div class="color-red"><i class="glyphicon glyphicon-alert"></i> ' . oLang:: _get('O_ATTEMPT_TO_UPLOAD_VIRUSFILE') . '</div>';
                break;
        }
        return $status;
    }
    ...
    public function insertFileUploadLog($ip,$filename,$filetype,$filevalidationstatus,$virusscanstatus)
    {
        $datetime =  $this->getDateTime();
        $recordexist = $this->checkIfTheRecordAlreadyExists($ip,$filename,$filetype,$datetime);
        if(!$recordexist) {
            $vararray = array(
                'ip' => $ip,
                'filename' => $filename,
            ...
            $result = $this->db->addData('insert', '#__osefirewall_fileuploadlogv7', '', '', $vararray);
            $this->db->closeDBO();
            if ($result == 0) {
                oseFirewall::callLibClass('fwscannerv7','fwscannerv7');
                $fs = new oseFirewallScannerV7();
                $fs->errorLog('insertFileUploadLog', 'failed to insert the record in the file upload log db v7 table ');
            }
            return $result;
        }
    }

    public function updateFileUploadLog($ip,$vsscanstatus,$filename,$validationstatus = false)
    {
        //0 - not scanned
        //1 - scanned and malicious
        //2 - scanned and non malicious
        if($validationstatus == false)
        {
            $query = "UPDATE `#__osefirewall_fileuploadlogv7` SET  `virusscanstatus` = ".$vsscanstatus." WHERE `filename` = ".$this->db->quoteValue($filename)." AND `ip` LIKE ".$this->db->quoteValue($ip)." ORDER BY `id` DESC LIMIT 1";

        }else{
            $query = "UPDATE `#__osefirewall_fileuploadlogv7` SET  `virusscanstatus` = ".$vsscanstatus.",`validationstatus`= ".$validationstatus." WHERE `filename` = ".$this->db->quoteValue($filename)." AND `ip` LIKE ".$this->db->quoteValue($ip)." ORDER BY `id` DESC LIMIT 1";
    ...
    public function checkIfTheRecordAlreadyExists($ip,$filename,$filetype,$datetime)
    {
        $sql = 'SELECT COUNT(`id`) AS count FROM `#__osefirewall_fileuploadlogv7`WHERE `ip` ='.$this->db->quoteValue($ip).'AND `filename` = '.$this->db->quoteValue($filename).' AND `filetype` = '.$this->db->quoteValue($filetype).' AND `datetime` = '.$this->db->quoteValue($datetime);
        $this->db->setQuery($sql);
        $result = $this->db->loadObject();
        if(($result->count == 0))
        {
