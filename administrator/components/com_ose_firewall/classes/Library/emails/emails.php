...
class oseFirewallemails {
	...
	public function sendemail($type, $content) {
		...
		$webmaster = oseFirewall::getConfiguration ( 'admin' );
		if (OSE_CMS == 'wordpress') {
			$receiptient = array ();
			if (isset($webmaster ['data'] ['adminEmail'])) {
			...
			if ($type=='secureKey') {
				...
				$receiptient [0]->name = $current_user->username;
				$receiptient [0]->email = $current_user->email;
			} else {
	            if (isset($webmaster ['data'] ['adminEmail'])) {
					$receiptient [0]->name = 'Administrator';
					$receiptient [0]->email = $webmaster ['data'] ['adminEmail'];
	...
	protected function getEmailByType($type) {
		...
		switch ($type) {
			...
			case 'secureKey' :
				...
				break;
			case 'vsscanClean' :
				$email->subject = 'Centrora Virus Scanning';
				break;
			case 'vsscanFound' :
				$email->subject = 'Centrora Virus Scanning: [Warning]';
				break;
            case 'gitbackupcomplted' :
                $email->subject = 'Centrora Git Backup Completed';
                break;
		}
    ...
    public function sendEMailV7($content,$subject,$receipient_email= false)
    {
        $result = false;
        $config_var = $this->getConfigVars ();
        oseFirewall::loadEmails ();
        ...
        $emailTemplate =oseFirewallBase::loadEmailTemplate();
        $email->body = oseFirewallBase::formateTemplate($emailTemplate, $email->subject ,$content);
        $webmaster = oseFirewall::getConfiguration ( 'admin' );

        $receiptient = array ();
        if(!empty($receipient_email))
        {
            $receiptient [0] = new stdClass();
            $receiptient [0]->name = 'Administrator';
            $receiptient [0]->email =$receipient_email;
        }else{
            if (isset($webmaster ['data'] ['adminEmail'])) {
                $receiptient [0] = new stdClass();
                $receiptient [0]->name = 'Administrator';
