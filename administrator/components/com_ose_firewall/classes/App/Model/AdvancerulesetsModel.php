...
class AdvancerulesetsModel extends BaseModel
{
    ...
    public function downloadRequest($type)
    {
        ...
        if($type == "avs")
        {
            $download_result =  $update->downloadVirusPattern();
        }else {
            $download_result = $update->downloadFirewallAdvPatterns();
        }
        return $download_result;
    }


    public function installPatterns($type)
    {
        oseFirewall::callLibClass('update', 'updatePatterns');
        $update = new updatePattern();
        if($type == "avs")
        {
            $update_result =  $update->installVirusPatterns($type);
        }else {
            $update_result = $update->installFirewallpatterns($type);
        }
    ...
    public function checkPatternVersion($type)
    {
        oseFirewall::callLibClass('update', 'updatePatterns');
        $update = new updatePattern();
        $check=  $update ->checkPatternVersion($type);
        return $check;
    }
    ...
    public function getDatefromVirusCheckFile($type = false)
    {
        oseFirewall::callLibClass('update', 'updatePatterns');
        $update = new updatePattern();
        $check=  $update ->getDatefromVirusCheckFile($type);
        return $check;
    }
