...
class LoginController extends \App\Base {
	public function action_Validate () {
        ...
        {
            $domain = $this->model->getVar('domain', null);
            if(empty($domain))
            {
                $this->model->aJaxReturn(false, 'ERROR', "Domain cannot be empty", false);
            }else{
                if(substr($domain,-1)!= "/")
                {
                    $domain = $domain."/";
                ...
                {
                    $isValidated = \oseFirewallBase::suiteDomainValidation($result[1]);
                    if($isValidated == false)
                    {
                        $this->model->aJaxReturn(false, 'ERROR', "Please insert a valid domain , The domain name should be the same as the domain with the account name : centrorasuite", false);
                    }
                }else{
                    $this->model->aJaxReturn(false, 'ERROR', "The domain name is in incorrect format , Use format like : http://abc.com/ ".CONTACT_SUPPORT, false);
                }
            }
        }
		if (empty($website) || empty($email) || empty($password))
		{
            ...
            $result = $this->model->validate($website, $email, $password, $token,$domain);
            print_r($result);
            exit;
        }
		{
