...
class VlscanController extends \App\Base {
	...
	public function action_vlscan() {
		$this->model->loadRequest();
		$step= $this->model->getInt('step');
		//$type= $this->model->getInt('type');
		$results = $this ->model->vlscan($step);
		$this->model->returnJSON($results);
	}

    public function action_getLastScanRecord() {
        $results = $this ->model->getLastScanResult();
