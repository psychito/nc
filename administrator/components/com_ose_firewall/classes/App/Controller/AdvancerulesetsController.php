...
class AdvancerulesetsController extends \App\Base {
    ...
    public function action_installPatterns()
    {
        $this->model->loadRequest();
        $type = $this->model->getVar('type', null);
        $result = $this->model->installPatterns($type);
        $this->model->returnJSON($result);
    }


    public function action_checkPatternVersion()
    {
        $this->model->loadRequest();
        $type = $this->model->getVar('type', null);
        $result = $this->model->checkPatternVersion($type);
        $this->model->returnJSON($result);
    }
    ...
    {
        $this->model->loadRequest();
        $type = $this->model->getVar('type', false);
        $result = $this->model->getDatefromVirusCheckFile($type);
        $this->model->returnJSON($result);
    }

    public function action_checkUserType()
    {
        if(\oseFirewallBase::checkSubscriptionStatus(false))
        {
            $result = \oseFirewallBase::prepareSuccessMessage("Premium");
        }
    else{
        $result = \oseFirewallBase::prepareErrorMessage("Free");
        }
        $this->model->returnJSON($result);
    }
