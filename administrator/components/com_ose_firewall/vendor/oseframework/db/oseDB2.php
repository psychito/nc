...
if (!class_exists('oseDB2', false))
{
    abstract class oseDB2
    {
        ...
        public function getLastCommitInDB()
        {
            $tablename = '#__osefirewall_gitlog';
            $query = "SELECT * FROM `" .$tablename."` ORDER BY id DESC LIMIT 1 ";
            $this->setQuery($query);
            $result = $this->loadResult();
            return $result;
        }
        public function commitExistsInLog($commitid)
        {
            $tablename = '#__osefirewall_gitlog';
            $query = "SELECT * FROM `" .$tablename."` WHERE `commit_id`=".$this->quoteValue($commitid);
            $this->setQuery($query);
            $result = $this->loadResult();
            if(empty($result))
            {
        ...
        {
            $this->setQuery ("DROP VIEW IF EXISTS ".$this->QuoteTable($table));
            $this->dbo->exec('SET FOREIGN_KEY_CHECKS = 0;');
            $this->stm = $this->dbo->prepare($this->query);
            return $this->stm->execute();
        }
