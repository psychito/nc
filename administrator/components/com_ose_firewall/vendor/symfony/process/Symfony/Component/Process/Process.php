...
require_once (dirname(__FILE__).'/Exception/ExceptionInterface.php');
require_once (dirname(__FILE__).'/Exception/RuntimeException.php');
require_once (dirname(__FILE__).'/Exception/ProcessTimedOutException.php');
require_once (dirname(__FILE__).'/Exception/ProcessFailedException.php');
require_once (dirname(__FILE__).'/Exception/LogicException.php');
require_once (dirname(__FILE__).'/Exception/InvalidArgumentException.php');
require_once (dirname(__FILE__).'/ProcessPipes.php');
...
class Process
{
    ...
    public function __construct($commandline, $cwd = null, array $env = null, $input = null, $timeout = 60, array $options = array())
    {
        if (!function_exists('proc_open')) {
            throw new RuntimeException('The Process class relies on proc_open, which is not available on your PHP installation. Please enable it and you can enjoy the benefits of Git to backup your website.');
        }
    ...
    public function start($callback = null)
    {
        ...
        if (!is_resource($this->process)) {
            ...
            $this->addOutput(stream_get_contents($this->processPipes->pipes[1]));
            //fclose($this->processPipes->pipes[1]);
        }
    ...
    public function wait($callback = null)
    {
        ...
        if ($this->processInformation['signaled'] && $this->processInformation['termsig'] !== $this->latestSignal) {
            if ($this->processInformation['termsig']==11 ) {
                echo 'Git is not installed in the server, please contact the hosting company to install it, it takes 5 seconds for them to install Git into the server.<br/> Using Git t'; exit;
            }
            else {
                echo sprintf('The process has been signaled with signal "%s".', $this->processInformation['termsig']); exit;
            }
        }
